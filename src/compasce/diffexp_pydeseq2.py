...
def compute_pseudobulk(ladata, cm):
    ...
    for cell_type_col in cell_type_cols:
        ...
        pdata = dc.pp.pseudobulk(ladata, sample_col=cm.sample_id_col, groups_col=cell_type_col, layer="counts", mode="sum", verbose=True)
        
        # For cell type vs. rest, we use a design such as "~subclass_l1"
        pds2 = pt.tl.PyDESeq2(adata=pdata, design=f"~{cell_type_col}")
        pds2.fit()
        ...
        for cell_type in cell_types:
            ...
            df = pds2.test_contrasts(pds2.contrast(column=cell_type_col, baseline=cell_type, group_to_compare="Anti-PD-L1+Chemo"))
